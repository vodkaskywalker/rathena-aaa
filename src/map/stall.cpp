...
TIMER_FUNC(stall_init){
	...
	struct s_stall_data *st = nullptr;
	int i;
	...
	while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
		...
		Sql_GetData(mmysql_handle, 0, &data, nullptr); st->vender_id = atoi(data);
		Sql_GetData(mmysql_handle, 1, &data, nullptr); st->unique_id = atoi(data);
		Sql_GetData(mmysql_handle, 2, &data, nullptr); st->owner_id = atoi(data);
		st->bl.id = st->vender_id;
		Sql_GetData(mmysql_handle, 3, &data, nullptr); st->type = atoi(data);
		Sql_GetData(mmysql_handle, 4, &data, nullptr); st->vd.class_ = atoi(data);
		Sql_GetData(mmysql_handle, 5, &data, nullptr); st->vd.sex = (data[0] == 'F') ? SEX_FEMALE : SEX_MALE;
		...
		Sql_GetData(mmysql_handle, 7, &data, nullptr); st->bl.x = atoi(data);
		Sql_GetData(mmysql_handle, 8, &data, nullptr); st->bl.y = atoi(data);
		Sql_GetData(mmysql_handle, 9, &data, &len); safestrncpy(st->message, data, zmin(len + 1, MESSAGE_SIZE));
		Sql_GetData(mmysql_handle, 10, &data, nullptr); st->vd.hair_style = atoi(data);
		Sql_GetData(mmysql_handle, 11, &data, nullptr); st->vd.hair_color = atoi(data);
		Sql_GetData(mmysql_handle, 12, &data, nullptr); st->vd.body_style = atoi(data);
		Sql_GetData(mmysql_handle, 13, &data, nullptr); st->vd.weapon = atoi(data);
		Sql_GetData(mmysql_handle, 14, &data, nullptr); st->vd.shield = atoi(data);
		Sql_GetData(mmysql_handle, 15, &data, nullptr); st->vd.head_top = atoi(data);
		Sql_GetData(mmysql_handle, 16, &data, nullptr); st->vd.head_mid = atoi(data);
		Sql_GetData(mmysql_handle, 17, &data, nullptr); st->vd.head_bottom = atoi(data);
		Sql_GetData(mmysql_handle, 18, &data, nullptr); st->vd.robe = atoi(data);
		Sql_GetData(mmysql_handle, 19, &data, nullptr); st->vd.cloth_color = atoi(data);
		Sql_GetData(mmysql_handle, 20, &data, &len); safestrncpy(st->name, data, zmin(len + 1, MESSAGE_SIZE));
		Sql_GetData(mmysql_handle, 21, &data, nullptr); st->expire_time = strtoul(data, nullptr, 10);
		st->bl.type = BL_STALL;
		stall_db.push_back(st);
	}
		...
		switch(itStalls->type){
			case 0:{
				...
				while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
					char* data;
					struct item item;
					Sql_GetData(mmysql_handle, 0, &data, nullptr); item.nameid = strtoul(data, nullptr, 10);
					Sql_GetData(mmysql_handle, 1, &data, nullptr); item.amount = atoi(data);
					Sql_GetData(mmysql_handle, 2, &data, nullptr); item.identify = atoi(data);
					Sql_GetData(mmysql_handle, 3, &data, nullptr); item.refine = atoi(data);
					Sql_GetData(mmysql_handle, 4, &data, nullptr); item.attribute = atoi(data);
					for( i = 0; i < MAX_SLOTS; ++i ){
						Sql_GetData(mmysql_handle, 5+i, &data, nullptr);
						item.card[i] = atoi(data);
					}
					for( i = 0; i < MAX_ITEM_RDM_OPT; ++i ) {
						Sql_GetData(mmysql_handle, 5+MAX_SLOTS+i*3, &data, nullptr); item.option[i].id = atoi(data);
						Sql_GetData(mmysql_handle, 5+MAX_SLOTS+i*3, &data, nullptr); item.option[i].value = atoi(data);
						Sql_GetData(mmysql_handle, 5+MAX_SLOTS+i*3, &data, nullptr); item.option[i].param = atoi(data);
					}
					Sql_GetData(mmysql_handle, 5+MAX_SLOTS+MAX_ITEM_RDM_OPT*3, &data, nullptr); item.expire_time = strtoul(data, nullptr, 10);
					Sql_GetData(mmysql_handle, 6+MAX_SLOTS+MAX_ITEM_RDM_OPT*3, &data, nullptr); item.bound = atoi(data);
					Sql_GetData(mmysql_handle, 7+MAX_SLOTS+MAX_ITEM_RDM_OPT*3, &data, nullptr); item.unique_id = strtoull(data, nullptr, 10);;
					Sql_GetData(mmysql_handle, 8+MAX_SLOTS+MAX_ITEM_RDM_OPT*3, &data, nullptr); item.enchantgrade = atoi(data);

					Sql_GetData(mmysql_handle, 9+MAX_SLOTS+MAX_ITEM_RDM_OPT*3, &data, nullptr); itStalls->price[item_count] = atoi(data);
			...
			case 1: {
				...
				while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
					...
					Sql_GetData(mmysql_handle, 0, &data, nullptr); itStalls->itemId[item_count] = strtoul(data, nullptr, 10);
					Sql_GetData(mmysql_handle, 1, &data, nullptr); itStalls->amount[item_count] = atoi(data);
					Sql_GetData(mmysql_handle, 2, &data, nullptr); itStalls->price[item_count] = atoi(data);
